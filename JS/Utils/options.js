// Current language
let currentLang = 'bn';
let currentSourceLang = 'bangla'; // tracks which source group is shown

// Language translations
const translations = {
    en: {
        all: 'All',
        national: 'National',
        international: 'International',
        sports: 'Sports',
        tech: 'Tech',
        technology: 'Technology',
        filters: 'Filters',
        filterBySource: 'Filter by Source',
        banglaSources: 'Bangla Sources',
        englishSources: 'English Sources',
        applyFilters: 'Apply Filters',
        clearAll: 'Clear All',
        loadingNews: 'Loading news...',
        previous: 'Previous',
        next: 'Next',
        page: 'Page',
        noNewsAvailable: 'No news available',
        tryAdjusting: 'Try adjusting your filters or check back later',
        unableToLoad: 'Unable to load news',
        makeSureBackend: 'Please make sure the backend server is running',
        clusters: 'Clusters',
        articles: 'articles',
        viewArticles: 'View all articles',
        hideArticles: 'Hide articles',
        sources: 'Sources',
        addedOn: 'Added on',
        trending: 'Trending',
        clusters: 'Clusters',

        articles: 'articles',
        viewAllArticles: 'View all articles',
        hideArticles: 'Hide articles',
        sources: 'sources',
        coveredBy: 'Covered by',
        moreSources: 'more',
        showLess: 'Show Less',
        sources: 'sources',

        today: 'Today in 2 Minutes',
        topStories: 'Top Stories',
        updatedMinAgo: 'Updated minutes ago',
        articles: 'articles',
        sources: 'sources',
        topCoverage: 'Top Coverage',
        viewAllArticles: 'View all Headlines',

        coverageAnalysis: 'Coverage Analysis',
        learnMore: 'Learn more',
        howManySources: 'How many sources?',
        differentAngles: 'Different angles?',

        compareHeadlines: 'Compare Headlines Side-by-Side',
        showListView: 'Show List View',
        comparisonInfo: 'Highlighted words show how different outlets frame this story',

        banglaLabel: 'Bangla',
        englishLabel: 'English',

        tagline: 'Many Voices. Your Choice',

        shareStory: 'Share Story',
        copyLink: 'Copy Link',
        copyAsText: 'Copy as Text',
        copied: 'Copied!',
        copyFailed: 'Failed to copy',

        showDetails: 'Show details',
        hideDetails: 'Hide details',
    },
    bn: {
        all: 'সব',
        national: 'জাতীয়',
        international: 'আন্তর্জাতিক',
        sports: 'খেলাধুলা',
        tech: 'প্রযুক্তি',
        technology: 'প্রযুক্তি',
        filters: 'ফিল্টার',
        filterBySource: 'সূত্র অনুসারে ফিল্টার করুন',
        banglaSources: 'বাংলা উৎস',
        englishSources: 'ইংরেজি উৎস',
        applyFilters: 'ফিল্টার প্রয়োগ করুন',
        clearAll: 'সব মুছুন',
        loadingNews: 'খবর লোড হচ্ছে...',
        previous: 'আগের',
        next: 'পরবর্তী',
        page: 'পৃষ্ঠা',
        noNewsAvailable: 'কোনো খবর নেই',
        tryAdjusting: 'আপনার ফিল্টার সামঞ্জস্য করুন বা পরে আবার চেষ্টা করুন',
        unableToLoad: 'খবর লোড করা যায়নি',
        makeSureBackend: 'অনুগ্রহ করে নিশ্চিত করুন যে ব্যাকএন্ড সার্ভার চলছে',
        clusters: 'ক্লাস্টার',
        articles: 'নিবন্ধ',
        viewArticles: 'সব নিবন্ধ দেখুন',
        hideArticles: 'নিবন্ধ লুকান',
        sources: 'উৎস',
        addedOn: 'যোগ করা হয়েছে',
        trending: 'ট্রেন্ডিং খবর',
        clusters: 'ক্লাস্টার',

        trending: 'ট্রেন্ডিং খবর',
        articles: 'নিবন্ধ',
        viewAllArticles: 'সব নিবন্ধ দেখুন',
        hideArticles: 'নিবন্ধ লুকান',
        sources: 'উৎস',
        coveredBy: 'টি সূত্র',
        moreSources: 'আরও',
        showLess: 'কম দেখান',
        sources: 'সূত্র',

        today: 'আজকের ২ মিনিট',
        topStories: 'শীর্ষ খবর',
        updatedMinAgo: 'মিনিট আগে আপডেট',
        articles: 'নিবন্ধ',
        sources: 'সূত্র',
        topCoverage: 'শীর্ষ কভারেজ',
        viewAllArticles: 'সব নিবন্ধ দেখুন',

        coverageAnalysis: 'কভারেজ বিশ্লেষণ',
        learnMore: 'আরও জানুন',
        howManySources: 'কতগুলি সূত্র?',
        differentAngles: 'ভিন্ন দৃষ্টিকোণ?',

        compareHeadlines: 'শিরোনাম পাশাপাশি তুলনা করুন',
        showListView: 'তালিকা ভিউ দেখান',
        comparisonInfo: 'হাইলাইট করা শব্দগুলি দেখায় বিভিন্ন আউটলেট কীভাবে এই গল্পটি উপস্থাপন করে',

        banglaLabel: 'বাংলা',
        englishLabel: 'ইংরেজি',

        shareStory: 'শেয়ার করুন',
        copyLink: 'লিংক কপি করুন',
        copyAsText: 'টেক্সট কপি করুন',
        copied: 'কপি হয়েছে!',
        copyFailed: 'কপি করা যায়নি',

        showDetails: 'বিস্তারিত দেখুন',
        hideDetails: 'লুকান',
    }
};

// News sources organized by language
const NEWS_SOURCES = {
    bangla: {
        api: [
            'Prothom Alo',
            'Kaler Kantho',
            'Daily Noya Diganta',
            'Samakal',
            'Jugantor',
            'Amar Desh',
            'The Daily Ittefaq',
            'Daily Sangram',
            'Bangladesh Pratidin',
            'The Daily Inqilab',
            'Bonik Bartha (Bangla)',
            'The Business Standard (Bangla)',
            'Bangladesh Sangbad Sangstha (Bangla)',
            'Dhaka Tribune',
            'Jagonews24',
            'bd24live',
            'Amar Bangla BD',
            'Risingbd.com',
            'The New Nation (Bangla)',
            'Dhaka Mail',
            'channelionline',
            'Asian Tv Online',
            'Boishakhi Online',
            'khaborwala',
            'Bangla Tribune',
            'Just News BD (Bangla)',
            'Daily Janakantha',
            'Daily Amader Shomoi'
        ],
        en: [
            'Prothom Alo',
            'Kaler Kantho',
            'Daily Noya Diganta',
            'Samakal',
            'Jugantor',
            'Amar Desh',
            'The Daily Ittefaq',
            'Daily Sangram',
            'Bangladesh Pratidin',
            'The Daily Inqilab',
            'Bonik Bartha (Bangla)',
            'The Business Standard (Bangla)',
            'Bangladesh Sangbad Sangstha (Bangla)',
            'Dhaka Tribune',
            'Jagonews24',
            'bd24live',
            'Amar Bangla BD',
            'Risingbd.com',
            'The New Nation (Bangla)',
            'Dhaka Mail',
            'Channel I Online',
            'Asian Tv Online',
            'Boishakhi Online',
            'khaborwala',
            'Bangla Tribune',
            'Just News BD (Bangla)',
            'Daily Janakantha',
            'Daily Amader Shomoi'
        ],
        bn: [
            'প্রথম আলো',
            'কালের কণ্ঠ',
            'নয়া দিগন্ত',
            'সমকাল',
            'জুগান্তর',
            'আমার দেশ',
            'দৈনিক ইত্তেফাক',
            'দৈনিক সংগ্রাম',
            'বাংলাদেশ প্রতিদিন',
            'দ্য ডেইলি ইনকিলাব',
            'বণিক বার্তা (বাংলা)',
            'দ্য বিজনেস স্ট্যান্ডার্ড (বাংলা)',
            'বাংলাদেশ সংবাদ সংস্থা (বাংলা)',
            'ঢাকা ট্রিবিউন',
            'জাগোনিউজ24',
            'বিডি24লাইভ',
            'আমার বাংলা বিডি',
            'রাইজিংবিডি ডটকম',
            'দ্য নিউ নেশন (বাংলা)',
            'ঢাকা মেইল',
            'channelionline',
            'এশিয়ান টিভি অনলাইন',
            'বৈশাখী অনলাইন',
            'খবরওয়ালা',
            'বাংলা ট্রিবিউন',
            'জাস্ট নিউজ বিডি (বাংলা)',
            'দৈনিক জনকণ্ঠ',
            'দৈনিক আমাদের সময়'
        ]
    },
    english: {
        api: [
            'Prothom Alo (English)',
            'Bonik Bartha (English)',
            'The Business Standard (English)',
            'The Daily Ittefaq (English)',
            'The Daily Star',
            'The Financial Times',
            'The Daily Observer',
            'Bangladesh Sangbad Sangstha (English)',
            'Ars Technica',
            'investinglive.com',
            'Tech Crunch',
            'Wired',
            'The Hacker News',
            'The Verge',
            'The New Age',
            'The New Nation',
            'The Daily Times Of Bangladesh',
            'Bangladesh Post',
            'The Good Morning',
            'The Register',
            'Zeteo',
            'Al Jazeera',
            'BBC News',
            'TRT World',
            'Middle East Eye',
            'Business Insider',
            'Gizmodo',
            'Independent',
            'RT News',
            'Hindustan Times',
            'Just News BD (English)',
            'CNBC'
        ],
        en: [
            'Prothom Alo (English)',
            'Bonik Bartha (English)',
            'The Business Standard (English)',
            'The Daily Ittefaq (English)',
            'The Daily Star',
            'The Financial Times',
            'The Daily Observer',
            'Bangladesh Sangbad Sangstha (English)',
            'Ars Technica',
            'investinglive.com',
            'Tech Crunch',
            'Wired',
            'The Hacker News',
            'The Verge',
            'The New Age',
            'The New Nation',
            'The Daily Times Of Bangladesh',
            'Bangladesh Post',
            'The Good Morning',
            'The Register',
            'Zeteo',
            'Al Jazeera',
            'BBC News',
            'TRT World',
            'Middle East Eye',
            'Business Insider',
            'Gizmodo',
            'Independent',
            'RT News',
            'Hindustan Times',
            'Just News BD (English)',
            'CNBC'
        ],
        bn: [
            'প্রথম আলো (ইংরেজি)',
            'বণিক বার্তা (ইংরেজি)',
            'দ্য বিজনেস স্ট্যান্ডার্ড (ইংরেজি)',
            'দৈনিক ইত্তেফাক (ইংরেজি)',
            'দ্য ডেইলি স্টার',
            'দ্য ফিন্যান্সিয়াল টাইমস',
            'দ্য ডেইলি অবজার্ভার',
            'বাংলাদেশ সংবাদ সংস্থা (ইংরেজি)',
            'আরস টেকনিকা',
            'ইনভেস্টিং লাইভ ডট কম',
            'টেকক্রাঞ্চ',
            'ওয়্যার্ড',
            'দ্য হ্যাকার নিউজ',
            'দ্য ভার্জ',
            'নিউ এজ',
            'দ্য নিউ নেশন',
            'দ্য ডেইলি টাইমস অফ বাংলাদেশ',
            'দ্য বাংলাদেশ পোস্ট',
            'দ্য গুড মর্নিং ',
            'দ্য রেজিস্টার',
            'জেটিও',
            'আল জাজিরা',
            'বিবিসি নিউজ',
            'টিআরটি ওয়ার্ল্ড',
            'মিডল ইস্ট আই',
            'বিজনেস ইনসাইডার',
            'গিজমোডো',
            'Independent',
            'আরটি নিউজ',
            'হিন্দুস্তান টাইমস',
            'জাস্ট নিউজ বিডি (ইংরেজি)',
            'সিএনবিসি'
        ]
    }
};

const SOURCES_LINK = {
    'Prothom Alo': 'https://www.prothomalo.com/',
    'Kaler Kantho': 'https://www.kalerkantho.com/',
    'Daily Noya Diganta': 'https://www.dailynayadiganta.com/',
    'Samakal': 'https://samakal.com/',
    'Jugantor': 'https://www.jugantor.com/',
    'Amar Desh': 'https://www.dailyamardesh.com/',
    'The Daily Ittefaq': 'https://www.ittefaq.com.bd/',
    'Daily Sangram': 'https://dailysangram.com/',
    'Bangladesh Pratidin': 'https://www.bd-pratidin.com/',
    'The Daily Inqilab': 'https://dailyinqilab.com/',
    'Bonik Bartha (Bangla)': 'https://www.bonikbarta.com/',
    'The Business Standard (Bangla)': 'https://www.tbsnews.net/bangla/',
    'Bangladesh Sangbad Sangstha (Bangla)': 'https://www.bssnews.net/bangla/',
    'Dhaka Tribune': 'https://bangla.dhakatribune.com/',
    'Jagonews24': 'https://www.jagonews24.com/',
    'bd24live': 'https://www.bd24live.com/bangla/',
    'Amar Bangla BD': 'https://www.amarbanglabd.com/',
    'Risingbd.com': 'https://www.risingbd.com/',
    'The New Nation (Bangla)': 'https://bangla.dailynewnation.com/',
    'Dhaka Mail': 'https://dhakamail.com/',
    'channelionline': 'https://www.channelionline.com/',
    'Asian Tv Online': 'https://www.asiantvonline.com/',
    'Boishakhi Online': 'https://boishakhionline.com/',
    'khaborwala': 'https://bn.khaborwala.com/',
    'Bangla Tribune': 'https://www.banglatribune.com/',
    'Just News BD (Bangla)': 'https://www.justnewsbd.com/',
    'Daily Janakantha': 'https://www.dailyjanakantha.com/',
    'Daily Amader Shomoi': 'https://www.dainikamadershomoy.com/',

    'Prothom Alo (English)': 'https://en.prothomalo.com/',
    'Bonik Bartha (English)': 'https://en.bonikbarta.com/',
    'The Business Standard (English)': 'https://www.tbsnews.net/',
    'The Daily Ittefaq (English)': 'https://en.ittefaq.com.bd/',
    'The Daily Star': 'https://www.thedailystar.net/',
    'The Financial Times': 'https://thefinancialexpress.com.bd/',
    'The Daily Observer': 'https://observerbd.com/',
    'Bangladesh Sangbad Sangstha (English)': 'https://www.bssnews.net/',
    'Ars Technica': 'https://arstechnica.com/',
    'investinglive.com': 'https://investinglive.com/',
    'Tech Crunch': 'https://techcrunch.com/',
    'Wired': 'https://www.wired.com/',
    'The Hacker News': 'https://thehackernews.com/',
    'The Verge': 'https://www.theverge.com/',
    'The New Age': 'https://www.newagebd.net/',
    'The New Nation': 'https://dailynewnation.com/',
    'The Daily Times Of Bangladesh': '',
    'Bangladesh Post': 'https://tob.news/',
    'The Good Morning': 'https://thegoodmorning.net/',
    'The Register': 'https://www.theregister.com/',
    'Zeteo': 'https://zeteo.com/',
    'Al Jazeera': 'https://www.aljazeera.com/',
    'BBC News': 'https://www.bbc.com/',
    'TRT World': 'https://www.trtworld.com/',
    'Middle East Eye': 'https://www.middleeasteye.net/',
    'Business Insider': 'https://www.businessinsider.com/',
    'Gizmodo': 'https://gizmodo.com/',
    'Independent': 'https://www.independent.co.uk/',
    'RT News': 'https://www.rt.com/',
    'Hindustan Times': 'https://www.hindustantimes.com/',
    'Just News BD (English)': 'https://www.justnewsbd.com/en',
    'CNBC': 'https://www.cnbc.com/world/'
}