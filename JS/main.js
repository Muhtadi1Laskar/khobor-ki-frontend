...
function handleBottomNavClick(event, category) {
    ...
    // Sync with desktop nav tabs
    document.querySelectorAll('.nav-tab').forEach(tab => {
        tab.classList.remove('active');
    });
    const desktopTab = document.querySelector(`.nav-tab[data-category="${category}"]`);
    if (desktopTab) {
...
function handleMoreMenuClick(event, category) {
    event.preventDefault();
...
if (typeof toggleTheme === 'function') {
    toggleTheme = function() {
        ...
        if (mobileIcon && desktopIcon) {
            mobileIcon.textContent = desktopIcon.textContent;
        }
    };
}

// Sync language icon in more menu
const originalToggleLanguage = toggleLanguage;
if (typeof toggleLanguage === 'function') {
    toggleLanguage = function() {
        originalToggleLanguage();
        
        // Update mobile language icon
        const mobileIcon = document.getElementById('mobileLangIcon');
        const desktopIcon = document.getElementById('langIcon');
        if (mobileIcon && desktopIcon) {
            mobileIcon.textContent = desktopIcon.textContent;
        }
    };
...
document.addEventListener('click', (e) => {
    ...
    if (backdrop && backdrop.classList.contains('active')) {
        if (!menu.contains(e.target) && !e.target.closest('.bottom-nav-item')) {
            toggleMobileMenu();
        }
    }
});